...
Once the MCP server is connected, paste this read-only health-check prompt into your AI client. It exercises several query tools at once and gives you a one-shot view of your project's local config, cloud deployments, and secret drift.

<Prompt
  title="Project health check"
  description="Read-only audit across services, environments, and secret drift."
  prompt={`Give me a health check of my tsdevstack project using the tsdevstack MCP server.

1. Call list_services to show what's defined locally.
2. Call list_environments to see configured cloud environments.
3. For each environment, call infra_status and list_deployed_services.
4. For the dev environment, call diff_secrets to flag any drift between local and cloud.

Summarize the findings as a punch list (in sync vs. needs attention) and recommend next actions. Do NOT call any mutating tools — read-only only.`}
/>
