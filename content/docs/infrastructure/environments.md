...
5. **Add GitHub secrets** for CI (environment name in uppercase):
   ...
   `NPM_TOKEN` (if your project uses private npm packages) is the only generated-workflow secret that is **not** per-environment — add it once globally, no `_STAGING` / `_PROD` variant. See [Private npm packages](/docs/infrastructure/cicd-setup#private-npm-packages).
