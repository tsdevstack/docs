...
- Cloud environment deployed ([Infrastructure setup](/docs/infrastructure/architecture))
...
To test with real emails locally, set `EMAIL_PROVIDER` to `resend` in `.secrets.user.json` (see [Local Secrets](/docs/secrets/local-secrets)):
...
These records are separate from the infrastructure DNS records covered in [Domain Setup](/docs/infrastructure/domain-setup).
...
:::tip
Set up Resend domain verification alongside your [infrastructure DNS records](/docs/infrastructure/domain-setup#dns-records). You'll be adding DNS records for both at the same time, so doing them together avoids waiting for propagation twice.
:::
...
For the full API reference (email options, custom providers, testing), see [NotificationModule in nest-common](/docs/packages/nest-common#notifications).
...
See [Custom Email Provider](/docs/customization/email-provider) for a full walkthrough with code examples, secrets setup, and testing patterns.
