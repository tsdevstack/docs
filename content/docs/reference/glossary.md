...
| **Bucket** | An object storage container for files. [MinIO](https://min.io/) locally, S3 (AWS), Cloud Storage (GCP), or Blob Storage (Azure) in the cloud. Added via `add-bucket-storage`. See [Object Storage](/docs/features/object-storage). |
| **Terraform** | [Terraform](https://www.terraform.io/) by HashiCorp — the infrastructure-as-code tool used under the hood. You never write Terraform directly; the framework generates and applies it. |
...
| **Secrets files** | `.secrets.*.json` files containing environment-specific secrets. See [How Secrets Work](/docs/secrets/how-secrets-work). |
...
| **Scheduled job** | A job that runs on a cron schedule. Cloud Scheduler (GCP), EventBridge (AWS), or Container App Jobs (Azure). Configured in `.tsdevstack/infrastructure.json` and deployed via `deploy-scheduler`. See [Scheduled Jobs](/docs/features/scheduled-jobs). |
...
| **`NPM_TOKEN`** | Single global GitHub repository secret (not per-env) for authenticating against private npm registries during host `npm ci` and tsdevstack-spawned docker builds. Auto-detected when `.npmrc` exists at project root. See [Private npm packages](/docs/infrastructure/cicd-setup#private-npm-packages). |
...
| **MCP** | [Model Context Protocol](https://modelcontextprotocol.io/) — the standard for AI agent tool integration. The built-in MCP server exposes 54 tools and 12 resources so agents like Claude Code can deploy, query, and debug your project. See [MCP Server](/docs/features/mcp-server). |
