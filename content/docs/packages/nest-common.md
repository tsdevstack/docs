...
For deeper coverage, see [How Secrets Work](/docs/secrets/how-secrets-work).
...
Provider is selected by the `EMAIL_PROVIDER` secret. For production setup (account creation, domain verification, DNS records), see [Resend setup](/docs/integrations/resend).

### Custom email provider

You can replace Resend with SendGrid, Mailgun, Postmark, or any other provider by implementing the `EmailProvider` interface and overriding the `EMAIL_PROVIDER` token. See [Custom Email Provider](/docs/customization/email-provider) for a full walkthrough.
...
For setup instructions, bucket naming, secrets, and cloud deployment details, see [Object Storage](/docs/features/object-storage).
...
For topic management, naming conventions, and flow diagrams, see [Async Messaging](/docs/features/async-messaging).
...
For configuration, custom metrics, manual spans, and cloud setup, see [Observability](/docs/features/observability).
