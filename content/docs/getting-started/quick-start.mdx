...
See [Running Locally](/docs/local-development/running-locally) for details.
...
|----------|---------------|-------|
| **GCP** | Service account + JSON key | [GCP Setup](/docs/infrastructure/providers/gcp/account-setup) |
| **AWS** | IAM user + access key (separate account per env) | [AWS Setup](/docs/infrastructure/providers/aws/account-setup) |
| **Azure** | App Registration + client secret | [Azure Setup](/docs/infrastructure/providers/azure/account-setup) |
...
See [Domain Setup](/docs/infrastructure/domain-setup) for details.
...
If your project uses private npm packages, also add a single `NPM_TOKEN` repo secret (global, not per-env) and create an `.npmrc` at project root with `${NPM_TOKEN}` interpolation — tsdevstack auto-detects and threads it through generated workflows and docker builds. See [Private npm packages](/docs/infrastructure/cicd-setup#private-npm-packages).

See [CI/CD Setup](/docs/infrastructure/cicd-setup) and the provider-specific CI/CD pages for details.
